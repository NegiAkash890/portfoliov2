...
function Timeline({activeIndex, data, setActiveIndex}) {
  const move = (type) => {
    ...
    if(activeIndex == data?.length - 1 && type === 'next') {
        ...
        switch(type) {
        case 'prev' : setActiveIndex(activeIndex - 1) ;
        return; 
        case 'next' : setActiveIndex(activeIndex + 1) ;
        return;
...
//   },[activeIndex]);
  ...
  return (
        ...
        {data.map((item, index) => {return (
                ...
                <li key={index} className={activeIndex >= index ? styles["timeline_step"] :styles["timeline_step_disabled"]}>
        ...
        <li><a className={styles["prev"]} onClick={() => move('prev')}>&lt;</a></li>
        <li><a className={styles["next"]} onClick={() => move('next')}>&gt;</a></li>
        </ul> 
        <div className={styles["events-content"]}>
            <div className={styles["selected"]} data-date="16/01/2014">
                <em>{data[activeIndex].date}</em>
                <Text fontSize={"1.2rem"} minH="250px">	
